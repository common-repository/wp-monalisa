...
function wpml_admin() {
	...
	if ( isset( $_POST['action'] ) && 'editicons' == $_POST['action'] && isset( $_POST['baction'] ) ) {
        // check nonce.
		if ( ! isset( $_POST['wpm_nonce2'] ) ) {
			die( "<br><br>Looks like you didn't send any credentials. Please reload the page. " );
		}
		if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['wpm_nonce2'] ) ), 'wpm_nonce2' ) ) {
			die( "<br><br>Looks like you didn't send any credentials. Please reload the page. " );
		}
    ...
    $out .= '<input name="wpm_nonce2" id="wpm_nonce2" type="hidden" value="' . esc_attr( wp_create_nonce( 'wpm_nonce2' ) ) . '" />';
