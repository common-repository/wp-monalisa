...
function wp_monalisa_init() {
	...
	if ( ! wpml_is_amp() ) {
		...
		if ( array_key_exists( 'richeditor', $av ) && '1' == $av['richeditor'] ) {
			...
			$erg1      = wp_add_inline_script( 'wpml_script', 'window._wpml_richedit_smiliesperrow = ' . json_encode( $av['smiliesperrow'] ) . ';', 'before' );
			$erg2      = wp_add_inline_script( 'wpml_script', 'window._wpml_richedit_maxwidth = ' . json_encode( $av['wpmlmaxwidth'] ) . ';', 'before' );
			$erg3      = wp_add_inline_script( 'wpml_script', 'window._wpml_richedit_maxheight = ' . json_encode( $av['wpmlmaxheight'] ) . ';', 'before' );
